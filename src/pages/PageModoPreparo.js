...
function ehChurrasco(id) {
  if (id === "12" || id === "13" || id === "14")
    localStorage.setItem(
      "churrasco",
      parseInt(localStorage.getItem("churrasco")) + 1
    );
...
function ehEpocaAno(id) {
  if (
    ...
    localStorage.setItem(
      "epoca_ano",
      parseInt(localStorage.getItem("epoca_ano")) + 1
    );
...
function ehHalloween(id) {
  if (id === "10" || id === "11")
    localStorage.setItem(
      "halloween",
      parseInt(localStorage.getItem("halloween")) + 1
    );
...
function getCasamento() {
  const casamento = {
    ...
    imagem: Anel,
    descrição_conquista:
      "Está conquista é adquirida ao se completar o bolo de casamento",
  };
  return casamento;
}
...
function getChurrasco() {
  const casamento = {
    ...
    imagem: Carne,
    descrição_conquista:
      "Está conquista é adquirida ao se completar todas as receitas para churrasco",
  };
  return casamento;
}
...
function getNatal() {
  const casamento = {
    ...
    imagem: PaveImg,
    descrição_conquista:
      "Está conquista é adquirida ao se completar todas as receitas de natal",
  };
  return casamento;
}
...
function getHalloween() {
  const casamento = {
    ...
    imagem: HalloweenImg,
    descrição_conquista:
      "Está conquista é adquirida ao se completar todas as receitas de halloween",
  };
  return casamento;
}
...
function getEpocaAno() {
  const casamento = {
    ...
    imagem: EpocaImg,
    descrição_conquista:
      "Está conquista é adquirida ao se completar todas as receitas de época",
  };
  return casamento;
}
...
function getAlho() {
  const casamento = {
    ...
    imagem: rei_alho,
    descrição_conquista:
      "Está conquista é adquirida ao se completar todas as receitas que contém alho",
  };
  return casamento;
}
...
function PageModoPreparo() {
  ...
  const [play_churrasco] = useSound(churras_musica, { volume: 0.2 });
  const [play_demon] = useSound(demon, { volume: 0.5 });
  const [play_epoca] = useSound(musica_epoca, { volume: 0.5 });
  const [play_casamento] = useSound(musica_casamento, { volume: 0.2 });
  const [play_natal] = useSound(musica_natal, { volume: 0.2 });
  const [play_alho] = useSound(musica_rei_alho, { volume: 0.2 });
  const [play_halloween] = useSound(musica_halloween, { volume: 0.4 });
  ...
  function escolhe_musica() {
    ...
    // pave pa come
      ...
      localStorage.setItem(
        "natal",
        parseInt(localStorage.getItem("natal")) + 1
      );
      play_natal();
    }
    // halloween
